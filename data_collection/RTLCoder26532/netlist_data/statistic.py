...
def create_csv_from_graph_data(json_data, csv_file_path):
    ...
    with open(csv_file_path, mode='a', newline='') as file:
        ...
        for key, value in tqdm(json_data.items()):
            if int(key) <= 180876:
                continue
            with open(f"/home/weili3/VLSI-LLM/data_collection/RTLCoder26532/netlist_data/graph/{value['rtl_id']}_{value['synthesis_efforts']}.pkl", "rb") as graph_file:
...
def append_verilog_file_length(csv_file_path, json_data):
    temp_file_path = csv_file_path + '.tmp'
    with open(csv_file_path, mode='r', newline='') as infile, open(temp_file_path, mode='w', newline='') as outfile:
        reader = csv.reader(infile)
        writer = csv.writer(outfile)
        header = next(reader)
        if "verilog_file_length" not in header:
            header.append("verilog_file_length")
        writer.writerow(header)
        
        for row in tqdm(reader):
            key = row[0]
            value = json_data[key]
            verilog_file_path = f"/home/weili3/VLSI-LLM/data_collection/RTLCoder26532/netlist_data/verilog/{value['rtl_id']}_{value['synthesis_efforts']}.v"
            verilog_file_length = get_file_length(verilog_file_path)
            row.append(verilog_file_length)
            writer.writerow(row)
    
    os.replace(temp_file_path, csv_file_path)

if __name__ == "__main__":
    with open("/home/weili3/VLSI-LLM/data_collection/RTLCoder26532/netlist_data/netlist.json", "r") as file:
        data = json.load(file)
    #create_csv_from_graph_data(data, "/home/weili3/VLSI-LLM/data_collection/RTLCoder26532/netlist_data/netlist.csv")
    append_verilog_file_length("/home/weili3/VLSI-LLM/data_collection/RTLCoder26532/netlist_data/netlist.csv", data)